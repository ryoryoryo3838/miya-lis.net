...
// 元のコードで使用されていた変数を定義
let t = 0; // 時間経過用の変数
const w = 400; // キャンバスの幅と高さ

// スケッチ本体の関数
const complexSketch = (p) => {
  // 初期設定 (p.createCanvas, p.backgroundなどはここで行う)
  p.setup = () => {
    // キャンバスを作成
    p.createCanvas(w, w);
  };

  // 描画ループ (アニメーションの更新、描画などはここで行う)
  p.draw = () => {
    // 1. 時間変数の更新
    // p.PI は p5.js の定数 PI (円周率)
    t += p.PI / 120;
    ...
    // 3. 描画設定
    p.stroke(255, 96); // 線の色 (白に近い)
    p.strokeWeight(1); // 点を描くため線の太さを1に
...
const SampleSketch = () => {
  useEffect(() => {
    new p5(complexSketch); // p5.jsのキャンバスを生成
  }, []);
